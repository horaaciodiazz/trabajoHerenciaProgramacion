...
namespace Ucu.Poo.Twitter
{
    public class Tuple
    {
        ...
        public Tuple(HttpStatusCode code , string text)
        {
            this.Code = code;
            this.Text = text;
        }
    ...
    public class TwitterImage : TwitterApi
    {
        ...
        public string PublishToTwitter(string post, string pathToImage)
        {
            try
            {
                ...
                var rezImageJson = JObject.Parse(rezImage.Result.Text);

                if (rezImage.Result.Code != HttpStatusCode.OK)
                {
        ...
        private Task<Tuple> TweetImage(string pathToImage)
        {
        ...
        private async Task<Tuple> SendImage(string url, MultipartFormDataContent multipartContent)
        {
            using (var httpClient = new HttpClient())
            {
                ...
                return new Tuple(httpResponse.StatusCode, httpContent);
            }
        }
    }
}
